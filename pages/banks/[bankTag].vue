...
const getRating = (brand: any): string => {
  const commentary = brand?.commentary;
    ...
    transform: ({ brand }) =>
      brand
        ? {
            ...brand,
            ...brand.commentary,
            bankFeatures: (brand as any).bankFeatures,
...
// Fetch harvest data for interest rates
...
const getFieldOrDefault = (fieldName: string): string => {
  const fieldValue = (bankData.value as any)?.[fieldName];
  const trimmedText =
