...
<div class="contain grid grid-cols-1 md:grid-cols-5 gap-8">
    ...
    <div class="col-span-1 md:col-span-3 pt-1">
        <PrismicRichText v-if="switchSurveyText" :field="switchSurveyText"></PrismicRichText>
        <PrismicRichText class="font-semibold" v-if="switchSurveyLink" :field="switchSurveyLink"></PrismicRichText>
    </div>
</div>
</template>
...
const switchSurveyText : ComputedRef<RichTextField | null> = computed(() => {
    let result = props.prismicDefaultPageData != null && 
        props.prismicDefaultPageData['switch-survey-text']
    if (result && result[0] && typeof result[0].text == 'string' && props.bankName) 
...
const switchSurveyLink : ComputedRef<RichTextField | null> = computed(() =>
    props.prismicDefaultPageData != null && props.prismicDefaultPageData['switch-survey-link']
);
</script>