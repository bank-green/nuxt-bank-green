<template>
    <Modal v-if="surveyContent != null" v-model="showModal">
        <div class="flex flex-col items-center">
            <PrismicRichText v-if="surveyContent?.data?.title" :field="surveyContent.data.title"
                class="text-3xl mb-6 font-semibold whitespace-pre-wrap" />
            <PrismicRichText v-if="surveyContent?.data?.subtitle" :field="surveyContent.data.subtitle"
                class="text-lg mb-6 font-semibold whitespace-pre-wrap" />
            <NuxtLink @click="clickSurvey" :to="surveyContent?.data?.link?.url"
...
const { data: surveyContent } = await useAsyncData('switchsurveyexit', () => client.getSingle('switchsurveyexit'))
</script>
